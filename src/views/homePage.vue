<template>
	<div class="index">
				...
				<div class="attribute-box">
					...
					<div
						...
						@click="
							$notifys({
								title: '提示',
								message: `您距离${$levelNames(
									player.level + 1
								)}境界还需${$formatNumberToChineseUnit(
					...
					<div
						...
						@click="
							$notifys({
								title: '获得方式',
								message: '每转生一次可以增加50容量',
							})
						"
					>
						背包容量: {{ player.inventory.length }} /
				...
				<div class="attributes">
					<div class="attribute-box">
						...
						<div class="tag attribute">
							成功率:
							{{
								(calculateEnhanceSuccessRate(strengthenInfo) * 100).toFixed(2)
							}}%
						</div>
					</div>
				</div>
				...
				<div class="monsterinfo-box">
					<p>
						<span class="description"
							>类型: {{ $genre[inventoryInfo.type] }}</span
						>
						<span class="icon" />
						<span class="value" />
					</p>
					<p>
						<span class="description"
							>炼器: {{ inventoryInfo.strengthen || 0 }}</span
						>
						<span class="icon">
							<i
								:class="
					...
					<p>
						<span class="description"
							>境界: {{ $levelNames(inventoryInfo.level) }}</span
						>
						<span class="icon">
							<i
								:class="
					...
					<p>
						<span class="description"
							>品质: {{ $levels[inventoryInfo.quality] }}</span
						>
						<span class="icon">
							<i
								:class="
			...
			<el-collapse v-model="petCollapse" class="collapse">
				<el-collapse-item name="1">
					...
					<div class="monsterinfo-box">
						<p>
							<span class="description"
								>气血: {{ petInfo?.initial?.health }}</span
							>
							<span class="icon">
								<i
									:class="
						...
						<p>
							<span class="description"
								>攻击: {{ petInfo?.initial?.attack }}</span
							>
							<span class="icon">
								<i
									:class="
						...
						<p>
							<span class="description"
								>防御: {{ petInfo?.initial?.defense }}</span
							>
							<span class="icon">
								<i
									:class="
			...
			<el-collapse v-model="inventoryCollapse" class="collapse">
				<el-collapse-item name="1">
					...
					<div class="monsterinfo-box">
						<p>
							<span class="description"
								>气血: {{ inventoryInfo?.initial?.health }}</span
							>
							<span class="icon">
								<i
									:class="
						...
						<p>
							<span class="description"
								>攻击: {{ inventoryInfo?.initial?.attack }}</span
							>
							<span class="icon">
								<i
									:class="
						...
						<p>
							<span class="description"
								>防御: {{ inventoryInfo?.initial?.defense }}</span
							>
							<span class="icon">
								<i
									:class="
		...
		<el-dialog lock-scroll v-model="show" title="游戏设置" width="350px">
			<div class="dialog-footer">
		...
		>
			...
			>
			<el-button type="primary" @click="confirmCollectionNewBie"
				>领取装备</el-button
			>
		</el-drawer>
...
// 装备信息
import equipmentItem from "@/components/equipmentItem/index.vue"
...
export default {
	...
	methods: {
				...
				{ text: "开始修炼", handler: () => this.$router.push("/cultivate") },
				{
					...
					handler: () => {
						...
						if (this.player.level < 10) {
							this.$notifys({
								title: "实力不足提示",
								message: `外面太危险了, 请突破到${this.$levelNames(
									10
								)}再出去吧!`,
							})
							return
		...
		reset() {
			this.storyText =
				'属性出错, 请添加QQ群:920930589, 上传"存档"并联系作者解决'
		...
		async equipItem(id, type) {
			...
			if (JSON.stringify(this.player.equipment[type]) != "{}") {
				...
				// 重新刷新当前排序
				this.equipmentDropdown(this.equipmentDropdownActive)
			}
			// 装备新装备
		...
		petClose(item) {
			this.$confirm(
				`你确定要放生<span class="el-tag el-tag--${this.computePetsLevel(
					item.level
				)}">${item.name}(${this.$levelNames(item.level)})</span>吗?`,
				"灵宠放生通知",
				{
					center: true,
		...
		shopBuy(item) {
			if (this.player.props.currency >= this.shopPrice) {
				...
				this.$notifys({
					title: "购买提示",
					message: `您成功花费${this.shopPrice}鸿蒙石购买${item.name}`,
				})
			} else {
			...
			this.$confirm("", item.name, {
				...
				message: `<div class="monsterinfo">
                        <div class="monsterinfo-box">
                            ...
                            <p>气血: ${this.$formatNumberToChineseUnit(
															item.health
														)}</p>
                            <p>攻击: ${this.$formatNumberToChineseUnit(
															item.attack
														)}</p>
                            <p>防御: ${this.$formatNumberToChineseUnit(
															item.defense
														)}</p>
                        </div>
                    </div>`,
			...
			this.$confirm("", item.name, {
				...
				message: `<div class="monsterinfo">
                        <div class="monsterinfo-box">
                            ...
                            <p>装备评分: ${this.$formatNumberToChineseUnit(
															item.score
														)}</p>
                        </div>
                    </div>`,
		...
		inventoryClose(item) {
			this.$confirm(
				`你确定要分解<span class="el-tag el-tag--${item.quality}">${
					this.$levels[item.quality]
				}${item.name}(${this.$genre[item.type]})</span>吗?`,
				"装备分解通知",
				{
					center: true,
      ...
      */
			const content =
				type == "qq" ? "920930589" : "https://github.com/setube/vue-XiuXianGame"
		...
		achievementInfo(type, item) {
			...
			if (
				...
				message = `<p>累积灵石: ${this.$formatNumberToChineseUnit(
